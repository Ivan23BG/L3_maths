...
\begin{center}
    \begin{tikzpicture}
        \node (ps) at (0, 0) {Convergence presque sûre};
        \node (lp) at (2, 2) {Convergence sur \(\mathcal L^p\)};
        \node (p) at (4, 0) {Convergence en probabilité};
        \draw[->, blue] (ps) -- (lp) node[midway, above, sloped] {implique si domination};
        \draw[->, red] (lp) -- (p) node[midway, above, sloped] {implique};
        \draw[->, blue] (p) -- (ps) node[midway, above, sloped] {implique si extraction};
        \draw[->, red] (ps) -- (p) node[midway, above, sloped] {implique};
        \draw[->, blue] (lp) -- (ps) node[midway, above, sloped] {implique si extraction};
        \draw[->, blue] (p) -- (lp) node[midway, above, sloped] {implique si hypothèse de moment};
    \end{tikzpicture}
\end{center}
