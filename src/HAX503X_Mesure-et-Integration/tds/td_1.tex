...
\iftoggle{showsolutions}{
    \begin{td-sol}[]\,
        \begin{enumerate}
            ...
            On a donc démontré que pour tout \(A,B\in\scrP(X)\), on a 
            \(A\subset B \Leftrightarrow \comp{B} \subset \comp{A}\).
...
\begin{td-exo}[Limites supérieures et inférieures de suites et fonctions]\, % 4
    ...
    \begin{center}
        \input{../assets/tikz/01_cosinus_6.tex}
    \end{center}
\end{td-exo}
% ----- Solutions exo 4
\iftoggle{showsolutions}{
...
% ----- Exo 6
\begin{td-exo}
    Soit \({(A_n)}_{n\in\bbN}\) une suite de parties de \(X\). Pour tout \(n\in\bbN\), on note \(f_n=\one_{A_n}\).
    
    Montrer qu'il existe deux parties \(B,C\in\scrP(X)\) telles que 
...
% ----- Exo 9
\begin{td-exo}
    Soit \(E\) un ensemble et \(A,B\in\scrP(E)\) deux parties fixées. On considère l'application \(\varphi:\scrP(E)\to\scrP(A)\times\scrP(B)\) définie par
    \[
